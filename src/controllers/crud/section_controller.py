...
class SectionController(CRUDBaseController[Section]):
    ...
    async def _validate_refs(
        ...
        *,
        ...
        instructor_exists = await db.scalar(select(User.id).where(User.id == instructor_id))
    ...
    async def create_section(self, db: AsyncSession, payload: SectionCreate) -> Section:
        await self._validate_refs(db, course_id=payload.course_id, instructor_id=payload.instructor_id)
        return await self.create(db, payload.model_dump())

    async def update_section(self, db: AsyncSession, section: Section, payload: SectionUpdate) -> Section:
        ...
        await self._validate_refs(db, course_id=next_course_id, instructor_id=next_instructor_id)
